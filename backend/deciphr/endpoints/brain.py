...
from backend.deciphr.models.brain_model import BrainCreateRequest
...
async def get_brain_by_id(brain_id: str):
    url = f"{brain_base_url}/v1/brain/{brain_id}"

    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(url, headers=brain_headers)
            response.raise_for_status()  # Raise an HTTPError for bad responses
            logger.info(
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                url, headers=brain_headers, json=brain.model_dump()
            )
            response.raise_for_status()  # Raise an HTTPError for bad responses
